...
import time
from lidarRay2world import lidarRay2world
import setup
...
def compute_stereo(timestamp, robot_pose, MAP):
  path_l = 'data/stereo_images/stereo_left/{}.png'.format(timestamp)
  path_r = 'data/stereo_images/stereo_right/{}.png'.format(timestamp)
  ...
  selected_height_down = -0.5
  selected_height_up = 2.5

  lidar_P_camera = np.array([-0.8349+1.64239, 0.0126869+0.247401, -1.76416+1.58411])
  o_R_c = np.array([[0,-1,0],[0,0,-1],[1,0,0]])
  v_R_o = np.array([[-0.00680499, -0.0153215, 0.99985],[-0.999977, 0.000334627, -0.00680066],[-0.000230383, -0.999883, -0.0153234]])
  
  # compute w_R_v and w_T_v based on robot pose
  theta = robot_pose[2]
  w_R_v = np.array([[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]])
  w_T_v = np.array([robot_pose[0], robot_pose[1]])
  ...
  focal = 0.94916
  K = np.array([[775.3724, 0, 619.4731, 0],[0, 775.3724,257.1805,0],[0,0,0,368.4132]])
  world_map = np.zeros([560,1280,2])
  #rgb_map = np.zeros([560,1280,3])
  for j in range(560):
      for i in range(1280):
          # need to convert optical frame to world frame
          if disparity[j][i] > 0:
              optical_z = K[2][3] / disparity[j][i]
              optical_y = optical_z * (j - K[1][2]) / K[1][1]
              optical_x = optical_z * (i - K[0][2]) / K[0][0]
              
              P_v = np.matmul(v_R_o,(np.array([optical_x, optical_y, optical_z])-np.matmul(o_R_c, lidar_P_camera)))
              
              # add height constrain in this part
              if selected_height_down <= P_v[2] <= selected_height_up:
                  
                  P_v_hat = np.array([P_v[0], P_v[1]])
                  P_w = np.matmul(w_R_v, P_v_hat) + w_T_v
                  
                  world_map[j][i] = np.array(MAP.meter2cell(P_w[0], P_w[1]))
              else:
                  world_map[j][i] = np.nan
          else:
              world_map[j][i] = np.nan
              #rgb_map[j][i] = np.array([0,0,0])
             
  return world_map, image_l
...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  vp[0:2,:]       occupied x,y positions from range sensor in world frame (in physical unit)  
...
def test_mapCorrelation():
  ...
  ranges = lidar_data[0, :]
  #####
  Y = lidarRay2world(ranges, [0,0,np.pi/2])
  #####
  # take valid indices
  ...
  print('here is inGood')
  print(len(indGood))
  print('xis length {}'.format(len(xis)))
  ...
  print('total: {}'.format(sum(sum(MAP['map']))))
...
if __name__ == '__main__':
  aa, bb = compute_stereo(1544582648735466220, [0,0,0])
  #show_lidar()
  #test_mapCorrelation()
  #test_bresenham2D()
