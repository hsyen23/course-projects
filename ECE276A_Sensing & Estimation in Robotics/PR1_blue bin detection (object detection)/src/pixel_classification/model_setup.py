...
class ModelSetup():
    ...
    def get_trainingData(self):
        ...
        '''
        # get R part
        data_value = read_pixels('data/training/red')
        l = len(data_value)
        label_value = np.zeros([l, 3])
        label_value[:, 0] = 1
        # get G part
        g_part = read_pixels('data/training/green')
        l = len(g_part)
        g_label = np.zeros([l, 3])
        g_label[:, 1] = 1
        data_value = np.concatenate((data_value, g_part), axis=0)
        label_value = np.concatenate((label_value, g_label), axis=0)
        # get B part
        b_part = read_pixels('data/training/blue')
        l = len(b_part)
        b_label = np.zeros([l, 3])
        b_label[:, 2] = 1
