...
class landmark:
    def __init__(self, number_of_landmark):
        ...
        self.landmark_covariance = np.empty([3 * number_of_landmark + 6, 3 * number_of_landmark + 6])
        self.landmark_covariance[:,:] = 0
        self.observed_landmark = set()
        self.number_of_landmark = number_of_landmark
        # hyperparameter
        self.observation_noise_factor = 0.5;
        self.landmark_prior_covariance_factor = 1;
        # observation noise need to be define inside loop beacuse the size keep change
        '''
        self.landmark_covariance[-6:,-6:] = np.block([
        [0.3*np.eye(3), np.zeros([3,3])],
        [np.zeros([3,3]), 0.05*np.eye(3)]
        ]
        )
        '''
    def update_robot_state_covariance(self, s_covariance):
        self.landmark_covariance[-6: , -6:] = s_covariance
    ...
    def update_landmark_prior(self, current_feature, current_robot_pose, c_T_imu, K, b):
        ...
        for i in range(current_feature.shape[1]):
            ...
            '''
            # filter out very far way prior
            robot_position = current_robot_pose[0:3, 3]
            landmark_position = world_frame[0:3]
            distance = norm(robot_position - landmark_position)
            if distance >= 900:
                continue
            '''
            ...
            self.landmark_covariance[i*3: (i+1)*3, i*3: (i+1)*3] = self.landmark_prior_covariance_factor*np.eye(3)
            #print(self.landmark_covariance[3*i : 3*(i+1), 3*i : 3*(i+1)])
            # add to observed set
    ...
    def kalman_filter_update(self,current_feature, current_robot_pose, c_T_imu, K, b):
        ...
        Overall_H = np.block([Overall_H, detached_H_for_state])
        ...
        fraction_covariance = np.matmul((np.eye(3*number_of_observed+6) - np.matmul(kalman_gain, Overall_H)) , fraction_covariance)
        for obs_index in range(number_of_observed):
            cor_index = landmark_record[obs_index]
        ...
        return np.matmul(current_robot_pose ,expm(sigle_twist_map(innovation[-6:])))
